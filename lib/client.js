...
FiveBeansClient.prototype.connect = function()
{
	...
	self.stream.on('data', function(data)
	{
		...
		{
			tmp = new Buffer.alloc(self.buffer.length + data.length);
...
var CRLF = new Buffer.from([0x0d, 0x0a]);
...
function makeBeanstalkCommand(command, expectedResponse, sendsData)
{
	...
	return function()
	{
		...
		if (data)
		{
			buffer = Buffer.concat([new Buffer.from(args.join(' ')), CRLF, data, CRLF]);
		}
		else
		{
			buffer = Buffer.concat([new Buffer.from(args.join(' ')), CRLF]);
		}
